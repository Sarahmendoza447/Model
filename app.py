...
def predict_attempt(data: AttemptFeatures):
    ...
    # --- Apply rule-based OBP band based on average score ---
    ...
    # --- Confidence by OBP group ---
    confidence_basic = class_proba.get("Basic", 0.0)
    confidence_intermediate = class_proba.get("Intermediate", 0.0)
    confidence_proficient = class_proba.get("Proficient", 0.0)
    confidence_exemplary = class_proba.get("Exemplary", 0.0)

    # Overall band group (for simplicity: Weak=Basic/Intermediate, Strong=Proficient/Exemplary)
    if pred_band in ["Basic", "Intermediate"]:
        band_group = "Weak"
    else:
        band_group = "Strong"
    ...
    return {
        ...
        "predicted_band": pred_band,             # Basic / Intermediate / Proficient / Exemplary
        "band_group": band_group,                # Weak / Strong
        "confidence_basic": confidence_basic,
        "confidence_intermediate": confidence_intermediate,
        "confidence_proficient": confidence_proficient,
        "confidence_exemplary": confidence_exemplary,
